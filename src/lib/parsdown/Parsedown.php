 ...
 * 
 * Parsedown recognizes that the Markdown syntax is optimized for humans so
...
class Parsedown
{
    ...
    // Lines
    //
    protected $BlockTypes = array('#' => array('Atx'),'*' => array('Rule','List'),'+' => array('List'),
        '-' => array('Setext','Table','Rule','List'),'0' => array('List'),'1' => array('List'),'2' => array('List'),
        '3' => array('List'),'4' => array('List'),'5' => array('List'),'6' => array('List'),'7' => array('List'),
        '8' => array('List'),'9' => array('List'),':' => array('Table'),'<' => array('Comment','Markup'),
        '=' => array('Setext'),'>' => array('Quote'),'_' => array('Rule'),'`' => array('FencedCode'),
        '|' => array('Table'),'~' => array('FencedCode'));
    ...
    protected function identifyCodeBlock($Line)
    {
        if ($Line['indent'] >= 4)
        {
            ...
            $Block = array(
                'element' => array('name' => 'pre','handler' => 'element',
                    'text' => array('name' => 'code','text' => $text)));
    ...
    {
        if (isset($Line['text'][3]) and $Line['text'][3] === '-' and $Line['text'][2] === '-' and
             $Line['text'][1] === '!')
        {
    ...
    protected function identifyList($Line)
    {
        ...
        {
            $Block = array('indent' => $Line['indent'],'pattern' => $pattern,
    ...
    protected function identifyQuote($Line)
    {
        ...
        {
            $Block = array(
                'element' => array('name' => 'blockquote','handler' => 'lines','text' => (array) $matches[1]));
    ...
    protected function addToMarkup($Line, array $Block)
    {
        ...
        if (stripos($Line['text'], '</' . $Block['name'] . '>') !== false) // closing
                                                                           // tag
        {
            if ($Block['depth'] > 0)
            {
    ...
    protected function identifyTable($Line, array $Block = null)
    {
        ...
        if (strpos($Block['element']['text'], '|') !== false and chop($Line['text'], ' -:|') === '')
        {
            ...
            $Block = array('alignments' => $alignments,'identified' => true,
            ...
            $Block['element']['text'][0]['text'][] = array('name' => 'tr','handler' => 'elements',
                'text' => $HeaderElements);
    ...
    protected function identifyReference($Line)
    {
        ...
        {
            $Definition = array('id' => strtolower($matches[1]),'data' => array('url' => $matches[2]));
    ...
    // Spans
    //
    protected $SpanTypes = array('!' => array('Link'),    // ?
    '&' => array('Ampersand'),'*' => array('Emphasis'),'/' => array('Url'),
        '<' => array('UrlTag','EmailTag','Tag','LessThan'),'[' => array('Link'),'_' => array('Emphasis'),
        '`' => array('InlineCode'),'~' => array('Strikethrough'),'\\' => array('EscapeSequence'));
    ...
    public function line($text)
    {
        ...
        while ($excerpt = strpbrk($remainder, $this->spanMarkerList))
        {
            ...
            foreach ($this->SpanTypes[$marker] as $spanType)
            {
                ...
                // Spans that start at the position of their marker don't have
                // to set a position.
    ...
    protected function identifyUrl($Excerpt)
    {
        ...
        if (preg_match('/\bhttps?:[\/]{2}[^\s<]+\b\/*/ui', $Excerpt['context'], $matches, PREG_OFFSET_CAPTURE))
        {
            ...
            return array('extent' => strlen($matches[0][0]),'position' => $matches[0][1],
    ...
    protected function identifyStrikethrough($Excerpt)
    {
        ...
        {
            return array('extent' => strlen($matches[0]),
    ...
    protected function identifyUrlTag($Excerpt)
    {
        if (strpos($Excerpt['text'], '>') !== false and
             preg_match('/^<(https?:[\/]{2}[^\s]+?)>/i', $Excerpt['text'], $matches))
        {
    ...
    protected function identifyEmailTag($Excerpt)
    {
        ...
        {
            return array('extent' => strlen($matches[0]),
                'element' => array('name' => 'a','text' => $matches[1],
    ...
    protected function identifyLink($Excerpt)
    {
        ...
        if ($Excerpt['text'][0] === '!')
        {
            $Element = array('name' => 'img','attributes' => array('alt' => $Link['text'],'src' => $url));
        }
        else
        {
            $Element = array('name' => 'a','handler' => 'line','text' => $Link['text'],
    ...
    protected $StrongRegex = array('*' => '/^[*]{2}((?:[^*]|[*][^*]*[*])+?)[*]{2}(?![*])/s',
        '_' => '/^__((?:[^_]|_[^_]*_)+?)__(?!_)/us');
    protected $EmRegex = array('*' => '/^[*]((?:[^*]|[*][*][^*]+?[*][*])+?)[*](?![*])/s',
        '_' => '/^_((?:[^_]|__[^_]*__)+?)_(?!_)\b/us');
    protected $textLevelElements = array('a','br','bdo','abbr','blink','nextid','acronym','basefont','b','em','big',
        'cite','small','spacer','listing','i','rp','del','code','strike','marquee','q','rt','ins','font','strong','s',
        'tt','sub','mark','u','xm','sup','nobr','var','ruby','wbr','span','time');
}
