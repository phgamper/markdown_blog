...
class Social
{
    ...
    public function socialButtons($link, $name = '', $desc = ''){
        ...
        if (array_key_exists('mail', $this->socials) && $this->socials['mail']) {
            $item = '<obfuscate><a href="mailto:?subject='.$name.'&amp;body='.$desc.'%0A%0A'.$link.'">';
            $item .= '<i class="fa fa-2x fa-envelope-square wow bounceIn" data-wow-delay=".6s" style="visibility: visible; animation-delay: 0.6s; animation-name: bounceIn;"> </i> </a></obfuscate>';
            $socials[] = $item;
        }
        return $socials;
    }
